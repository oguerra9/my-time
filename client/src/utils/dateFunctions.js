...
export const getWeekStart = (timestamp) => {
    ...
    let year = myTime.getFullYear();
    let month = myTime.getMonth();
    let dateNum = myTime.getDate();

    const weekDay = myTime.getDay();
    const currDate = new Date(year, month, dateNum, 0, 0, 0, 0);
    let msIntoWeek = weekDay * MS_PER_DAY;
    const currDateMS = currDate.getTime();
    const weekStartMS = currDateMS - msIntoWeek;
...
export const getWeekEnd = (timestamp) => {
    ...
    const weekEndMS = parseInt(weekStart) + weekMS - 1;
    return weekEndMS;
};
